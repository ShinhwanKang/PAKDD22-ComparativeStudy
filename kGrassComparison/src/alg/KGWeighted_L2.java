...
public class KGWeighted_L2 extends KGCommon {

    public int maxWeight=1;


    public KGWeighted_L2(String type){
        objectiveType = type;
    }
    ...
    public double getMergeCost(int A, int B){
        ...
        double denseCost;
        denseCost = w*w*(pi-edgeCnt) + (1-w)*(1-w)*edgeCnt;
    ...
    public double L2Error(){
        ...
        for(int n: snList){
            for(Int2IntOpenHashMap.Entry X:super2SuperEdgeCnt[n].int2IntEntrySet()){
                ...
                error += (w*w*(pi-edgeCnt)+(1-w)*(1-w)*edgeCnt);
