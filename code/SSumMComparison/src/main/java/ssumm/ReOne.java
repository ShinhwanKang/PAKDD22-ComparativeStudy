...
public class ReOne extends ssumm.SSumM {
    ...
    public double dataCostPair(int src, int dst, int n){
        ...
        if(modelType.equals("we")){
            ...
            sparse = sparseEdgeCost * edgeNum;
        }else{
            dense = (matrixSize - edgeNum) * sparseEdgeCost + 2 * log2(numSuperNodes);
            sparse = sparseEdgeCost * edgeNum;
        }
    ...
    public double norm(){
        ...
        for(int v: snList){
            for(Int2IntMap.Entry u: superGraph[v].int2IntEntrySet()){
                int edgeCount;
                if(modelType.equals("we")){
                    edgeCount = u.getIntValue();
                }
                else{
                    edgeCount = insideSupernode[u.getIntKey()].size();
                    if(v == u.getIntKey()){
                        edgeCount *= (edgeCount-1);
                    }
                    else edgeCount *= insideSupernode[v].size();
                }
